...
class Network(nn.Module):
    def __init__(self, config):
        ...
        self.conv_DME = conv(1, config.DME_dim, kernel_size=1)

        self.conv4_4 = nn.Sequential(
            conv(ch4 + config.DME_dim, ch4, kernel_size=ks),
            resnet_block(ch4, kernel_size=ks, res_num=4),
            conv(ch4, ch4+64, kernel_size=ks),
            conv(ch4+64, ch4, kernel_size=ks))

        self.conv_agg = nn.Sequential(
            conv(2 * ch4, ch4, kernel_size=ks),
            resnet_block(ch4, kernel_size=ks, res_num=5),
            resnet_block(ch4, kernel_size=ks, res_num=4),
            conv(ch4, ch4, kernel_size=ks),
            )
...
##########################################################################
    def forward(self, C, R=None, L=None, is_train=False):
        ...
        f = self.conv_agg(torch.cat([f, f_C], 1))
        f = self.conv_res(f)
        ...
        if is_train:
            ...
            f = self.kconv4_3(self.kconv4_2(self.kconv4_1(f)))
            DM = self.DME(f)
            f_R_warped = DPD(F.interpolate(R, scale_factor=1/8, mode='area'), DM, padding_mode = 'zeros')
            outs['f_R_w'] = f_R_warped
            outs['f_L'] = F.interpolate(L, scale_factor=1/8, mode='area')
